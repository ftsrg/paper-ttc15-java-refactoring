...
After creating the method signature traces, the following preconditions have to be met before pulling up a method:

\begin{itemize}[noitemsep]
	 \item every child class has a method with the given signature,
	 \item the parent class does not have it already,
	 \item and the transformation will not create unavailable method or field access.
\end{itemize}

%\noindent\ttctt{pattern possiblePUM(parentClass : TClass, methodSignatureTrace : MethodSignatureTrace)}

The main pattern, deciding, whether the \pum can be executed collects every parent class -- method signature pair, that satisfies the preconditions. The LHS is defined with one pattern, supported by 5 other. The execution is controlled by parameterising the main pattern using another two. (See \autoref{lst:PUMQueries.eiq}.) The RHS is defined in \autoref{lst:PUMTransformation.xtend}, where one utility pattern is also used.

\subsubsection{Create Superclass}
To create a new superclass, the list of the targeted classes have to be connected with a class list trace, virtually selecting them for the pattern. The transformation can be executed, if the preconditions are statisfied:

\begin{itemize}[noitemsep]
	\item the target parent class does not exist
	\item and every targeted child class has the same parent.
\end{itemize}

The LHS is defined in \autoref{lst:CSCQueries.eiq} with 1 main and 5 support patterns. The RHS is defined in \autoref{lst:CSCTransformation.xtend}, also using one utility pattern.
