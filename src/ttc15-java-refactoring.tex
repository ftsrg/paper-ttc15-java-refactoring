...
\autoref{fig:pg} shows the workflow of the transformation. The framework receives the path to the directory containing the source files. Each source files is treated as a \emph{compilation unit} and parsed using JDT~\ttccircled{1} to ASTs. Using JDT's resolving mechanism, these ASTs can be connected to an ASG.
...
To support efficient queries, we extended the program graph metamodel with a \emph{trace model}~(\autoref{fig:trace-mm}). The trace model is generated with a \viatra transformation (\ttccircled{3}, \ttccircled{4}).
...
\ttcfig{trace-mm}{Metamodel of the trace model}{0.7}
